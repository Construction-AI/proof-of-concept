...
import { FolderGit2, Plus, LogOut } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  // Pobieramy projekty zaraz po załadowaniu komponentu
  useEffect(() => {
    fetchProjects();
  }, []);
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewTitle(''); // Czyszczenie formularza
      fetchProjects(); // Odświeżenie listy po dodaniu
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-bold text-gray-900">Twoje Projekty</h2>
        </div>

        {/* Formularz szybkiego dodawania projektu */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <div 
                ...
                className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-300 cursor-pointer transition-all group"
              >
